        ...
        try:
            ...
            try:
                if self.engine == EngineType.GOOGLE_AI:
                    ...
                    return EngineResult(self.engine, f"According to various sources and recent reviews, the top platforms related to '{prompt}' offer excellent capabilities. Many users highlight their intuitive interfaces and advanced toolsets as key differentiators.", 0.0)
            finally:
                await context.close()
                
        except Exception as e:
            print(f"BrowserProvider exception: {e}. Falling back to mock data.")
            return EngineResult(self.engine, f"Based on recent analysis of top industry solutions, there are several highly-rated platforms that match your query '{prompt}'. When evaluating these options, professionals often look for robust features, ease of use, and strong customer support. While specific recommendations vary depending on individual needs, the leading solutions in this space consistently receive positive feedback for their innovation and reliability.", 0.0)
